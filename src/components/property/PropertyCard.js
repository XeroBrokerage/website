...
export default function PropertyCard({ property }) {
  return (
    <div className="bg-white/90 backdrop-blur-xs border border-gray-200/50 shadow-sm hover:shadow-md transition-all rounded-2xl overflow-hidden hover:-translate-y-1 w-full max-w-[350px] mx-auto">
      ...
      <div className="px-5 py-3 space-y-2 text-black">
        <h3 className="text-2xl poppins-bold tracking-wide">
          ₹{property.price?.toLocaleString("en-IN")}
        </h3>
        ...
        <div className="flex flex-wrap gap-3 mt-2 text-sm">
          <div className="px-4 py-2 rounded-full backdrop-blur-md flex items-center gap-3 text-black shadow-sm">
            <span className="font-medium">{property.beds} <span className="opacity-50">Beds</span></span>
            <span className="text-black/40">•</span>
            <span className="font-medium">{property.baths} <span className="opacity-50">Baths</span></span>
          </div>
        </div>
        ...
        <Link
          href={`/properties/${property._id}`}
          className="mt-3 inline-block w-full text-center text-sm font-medium px-3 py-2 bg-black text-white rounded-lg hover:bg-gray-800 transition-colors"
        >
          View Details
        </Link>
      </div>
