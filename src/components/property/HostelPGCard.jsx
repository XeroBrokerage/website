...
export default function HostelPropertyCard({ property }) {
  return (
      ...
      <div className="flex-1 px-5 py-3 flex flex-col justify-between gap-3 text-black">
        <div className="space-y-1">
          <h3 className="text-2xl poppins-bold tracking-wide truncate">
            ₹{property.rentPerMonth} Rent Per Month
          </h3>

          <h4 className="text-base poppins-semibold tracking-wide text-gray-800">
            {property.sharingType} sharing PG /Hostel for rent
          </h4>
          ...
          <div className="flex items-center gap-2 text-xs text-gray-500 mt-1">
            <span>{property.foodIncluded}</span>
            <span>•</span>
            <span>Attached Washroom : {property.attachedBathroom}</span>
            <span>•</span>
            <span>{property.acAvailable}</span>
          </div>
        </div>
